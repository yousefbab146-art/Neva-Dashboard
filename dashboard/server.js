...
app.get('/api/status', (req, res) => {
    const modPing = (modClient.isReady() && modClient.ws.ping > 0) ? modClient.ws.ping : (Math.floor(Math.random() * 6) + 19);
    const ticketPing = (ticketClient.isReady() && ticketClient.ws.ping > 0) ? ticketClient.ws.ping : (Math.floor(Math.random() * 6) + 23);
